// nagara_home.dart — Light/White Theme + Favourite filter + Search + Quick filters
// + Creative Quiz (hint, skip, gradient header, letter badges, small images)
...
List<ImageDetails> _images = const [
  ...
  ImageDetails(
      ...
      title:
          'ජාතික මාර්ග මංසන්ධියකට ලඟාවීමට පෙර දිශාවන් පෙන්වන මාර්ගයට ඉහලින් සවිකර ඇති පුවරුව',
      details:
          'Sign above the road showing directions before reaching a national highway junction'),
  ImageDetails(
...
class NagaraHome extends StatefulWidget {
  const NagaraHome({super.key});
  @override
  State<NagaraHome> createState() => _NagaraHomeState();
}

class _NagaraHomeState extends State<NagaraHome> {
  final _search = TextEditingController();
  // 0 All | 1 Overhead | 2 Before Junc. | 3 Lanes | 4 Area
  int _chip = 0;
  ...
  List<ImageDetails> get _filtered {
    ...
    switch (_chip) {
      case 1: // Overhead panels
        src = src.where((e) =>
            e.details.toLowerCase().contains('above the road') ||
            e.details.toLowerCase().contains('mounted above') ||
            e.title.contains('ඉහලින්'));
        break;
      case 2: // Before junction / approach
        src = src.where((e) =>
            e.details.toLowerCase().contains('before reaching') ||
            e.title.contains('ලඟාවීමට පෙර'));
        break;
      case 3: // Lanes
        src = src.where((e) =>
            e.details.toLowerCase().contains('lane') ||
            e.title.contains('මංතීරු'));
        break;
      case 4: // Area/Admin
        src = src.where((e) =>
            e.details.toLowerCase().contains('administrative area') ||
            e.title.contains('පරිපාලන'));
        break;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: const Text('දිශා තොරතුරු සංඥා',
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => NagaraQuizScreen(items: _images),
            ),
          );
        },
...
// ===================== CREATIVE QUIZ (NAGARA) =====================

class NagaraQuizScreen extends StatefulWidget {
  final List<ImageDetails> items;
  const NagaraQuizScreen({super.key, required this.items});

  @override
  State<NagaraQuizScreen> createState() => _NagaraQuizScreenState();
}

class _NagaraQuizScreenState extends State<NagaraQuizScreen> {
  // theme
  ...
  List<_Q> _buildQuestions(List<ImageDetails> items) {
    ...
    final total = min(6, pool.length); // small dataset -> 6 Qs
    final qs = <_Q>[];

    // randomize correct slots 0..3
        ...
        children: [
          ...
          Column(
            children: [
              ...
              // question card (small image + hint)
              Padding(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            const Text(
                              'මෙම දිශා/තොරතුරු පුවරුවේ සිංහල නම තෝරන්න:',
                              style: TextStyle(
                                  color: textPrimary,
                                  fontWeight: FontWeight.w900,
                                  fontSize: 16),
...
// ----------------- Result Screen -----------------
class NagaraQuizResult extends StatelessWidget {
  final int score;
  final int total;
  final List<ImageDetails> items;
  const NagaraQuizResult(
      {super.key,
      required this.score,
      required this.total,
      required this.items});
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                          builder: (_) => NagaraQuizScreen(items: items),
                        ),
                      );
                    },
