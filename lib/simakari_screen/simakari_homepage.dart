// simakari_home.dart — Restriction/Limit (සිමාකාරී) Signs
// Light/White Theme + Favourite filter + Smart Bottom Sheet
...
List<ImageDetails> _images = const [
  ImageDetails(
      imagePath: 'images/si1.png', title: 'පළල් සීමාව', details: 'Width Limit'),
  ImageDetails(
      imagePath: 'images/si2.png', title: 'උස සීමාව', details: 'Height Limit'),
  ImageDetails(
      imagePath: 'images/si3.png', title: 'බර සීමාව ', details: 'Weight Limit'),
  ImageDetails(
      imagePath: 'images/si4.png',
      title: 'ඇක්සලයක් මත බර සීමාව',
      details: 'Weight Limit on One Axle'),
  ImageDetails(
      imagePath: 'images/si5.png', title: 'වේග සීමාව', details: 'Speed Limit'),
  ImageDetails(
      imagePath: 'images/si6.png',
      title: 'වේග සීමාවේ අවසානය ',
      details: 'End of Speed Limit'),
];

// -------- Screen --------
class Simakari_Home extends StatefulWidget {
  const Simakari_Home({super.key});
  @override
  State<Simakari_Home> createState() => _Simakari_HomeState();
}

class _Simakari_HomeState extends State<Simakari_Home> {
  final _search = TextEditingController();
  int _chip = 0; // 0 All | 1 Size | 2 Weight | 3 Speed
  ...
  List<ImageDetails> get _filtered {
    ...
    switch (_chip) {
      case 1: // Size (width/height)
        src = src.where((e) =>
            e.details.toLowerCase().contains('width') ||
            e.details.toLowerCase().contains('height') ||
            e.title.contains('පළල්') ||
            e.title.contains('උස'));
        break;
      case 2: // Weight (weight/axle)
        src = src.where((e) =>
            e.details.toLowerCase().contains('weight') ||
            e.details.toLowerCase().contains('axle') ||
            e.title.contains('බර') ||
            e.title.contains('ඇක්සල'));
        break;
      case 3: // Speed
        src = src.where((e) =>
            e.details.toLowerCase().contains('speed') ||
            e.title.contains('වේග'));
        break;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: const Text('සිමාකාරී සංඥා',
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => SimakariQuizScreen(items: _images),
            ),
          );
        },
...
// ===================== CREATIVE QUIZ (RESTRICTION/LIMIT SIGNS) =====================

class SimakariQuizScreen extends StatefulWidget {
  final List<ImageDetails> items;
  const SimakariQuizScreen({super.key, required this.items});

  @override
  State<SimakariQuizScreen> createState() => _SimakariQuizScreenState();
}

class _SimakariQuizScreenState extends State<SimakariQuizScreen> {
  // theme constants
  ...
  // Sinhala distractors (generic road terms)
  ...
  late final List<_SimQ> _questions;
  int _current = 0;
  int _score = 0;
  ...
  List<_SimQ> _buildQuestions(List<ImageDetails> items) {
    final rng = Random();
    ...
    for (final correct in base) {
      ...
      qs.add(_SimQ(item: correct, options: opts, correct: correctIdx));
    }
    return qs;
  }
            ...
            children: [
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            const Text(
                              'මෙම සීමා/වැළැක්වීම් සංඥාවේ නිවැරදි සිංහල නම තෝරන්න:',
                              style: TextStyle(
                                  color: textPrimary,
                                  fontWeight: FontWeight.w900,
                                  fontSize: 16),
              ...
              Expanded(
                child: ListView.separated(
                  ...
                  itemBuilder: (_, i) => _SimAnswerTile(
                    index: i,
                    text: q.options[i],
...
class _SimAnswerTile extends StatelessWidget {
  final int index;
  final String text;
  final bool isCorrect;
  final bool isWrong;
  final VoidCallback onTap;
  const _SimAnswerTile({
    required this.index,
    required this.text,
...
class _SimQ {
  ...
  final int correct;
  _SimQ({required this.item, required this.options, required this.correct});
}

// ----------------- Result Screen -----------------
class SimakariQuizResult extends StatelessWidget {
  final int score;
  final int total;
  final List<ImageDetails> items;
  const SimakariQuizResult(
      {super.key,
      required this.score,
      required this.total,
      required this.items});
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                          builder: (_) => SimakariQuizScreen(items: items),
                        ),
                      );
                    },
