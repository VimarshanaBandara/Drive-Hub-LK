// palana.dart — Regulatory (පාලන) Signs — Light/White Theme + Favourite filter
...
class Palana extends StatefulWidget {
  const Palana({super.key});
  @override
  State<Palana> createState() => _PalanaState();
}

class _PalanaState extends State<Palana> {
  final _search = TextEditingController();
  int _chip = 0; // 0 All | 1 Turns | 2 Vehicle Types | 3 Parking
    ...
    switch (_chip) {
      case 1: // Turns
        src = src.where((e) =>
            ...
            e.title.contains('හැරවීම') ||
            e.details.toLowerCase().contains('u-turn'));
        break;
      case 2: // Vehicle-specific closures
        src = src.where((e) =>
            e.details.toLowerCase().contains('road closed') ||
            e.title.contains('වාහන') ||
            e.title.contains('බස්') ||
            e.title.contains('ලොරි') ||
            e.title.contains('යතුරුපැදි') ||
  ...
  Widget build(BuildContext context) {
    ...
    const accent = Color(0xFF7C4DFF); // your purple
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (_) => PalanaQuizScreen(items: _images)),
          );
        },
...
// ===================== CREATIVE QUIZ (REGULATORY) =====================

class PalanaQuizScreen extends StatefulWidget {
  final List<ImageDetails> items;
  const PalanaQuizScreen({super.key, required this.items});

  @override
  State<PalanaQuizScreen> createState() => _PalanaQuizScreenState();
}

class _PalanaQuizScreenState extends State<PalanaQuizScreen> {
  // theme constants
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            const Text(
                              'මෙම පාලන සංඥාවේ නිවැරදි සිංහල නම තෝරන්න:',
                              style: TextStyle(
                                  color: textPrimary,
                                  fontWeight: FontWeight.w900,
                                  fontSize: 16),
...
// ----------------- Result Screen -----------------
class PalanaQuizResult extends StatelessWidget {
  final int score;
  final int total;
  final List<ImageDetails> items;
  const PalanaQuizResult(
      {super.key,
      required this.score,
      required this.total,
      required this.items});
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                          builder: (_) => PalanaQuizScreen(items: items),
                        ),
                      );
                    },
