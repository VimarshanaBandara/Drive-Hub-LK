// vidhana_home.dart — Mandatory (විධාන) Signs
...
class Vidhana_home extends StatefulWidget {
  const Vidhana_home({super.key});
  @override
  State<Vidhana_home> createState() => _Vidhana_homeState();
}

class _Vidhana_homeState extends State<Vidhana_home> {
  final _search = TextEditingController();
  // 0 All | 1 Turn | 2 Ahead | 3 Pass | 4 Roundabout
  int _chip = 0;
  ...
  List<ImageDetails> get _filtered {
    ...
    switch (_chip) {
      case 1: // Turn (Left/Right/Left Ahead/Right Ahead)
        src = src.where((e) =>
            e.details.toLowerCase().contains('turn') ||
            e.title.contains('හැර'));
        break;
      case 2: // Ahead / Straight
        src = src.where((e) =>
            e.details.toLowerCase().contains('ahead') ||
            e.title.contains('ඉදිරිය') ||
            e.title.contains('කෙලින්ම'));
        break;
      case 3: // Pass
        src = src.where((e) =>
            e.details.toLowerCase().contains('pass') ||
            e.title.contains('පසු කරන්න'));
        break;
      case 4: // Roundabout
        src = src.where((e) =>
            e.details.toLowerCase().contains('roundabout') ||
            e.title.contains('වටරවුම'));
        break;
    }
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => VidhanaQuizScreen(items: _images),
            ),
          );
        },
...
// ===================== CREATIVE QUIZ (MANDATORY SIGNS) =====================

class VidhanaQuizScreen extends StatefulWidget {
  final List<ImageDetails> items;
  const VidhanaQuizScreen({super.key, required this.items});

  @override
  State<VidhanaQuizScreen> createState() => _VidhanaQuizScreenState();
}

class _VidhanaQuizScreenState extends State<VidhanaQuizScreen> {
  // theme
  ...
  late final List<_VQ> _questions;
  int _current = 0;
  int _score = 0;
  ...
  List<_VQ> _buildQuestions(List<ImageDetails> items) {
    final rng = Random();

    // dedupe identical titles if any
    ...
    for (final correct in base) {
      ...
      qs.add(_VQ(item: correct, options: options, correct: correctIdx));
    }
    return qs;
  }
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            ...
            right: -40,
            child: _blob(220,
                [const Color(0xFF22C55E).withOpacity(.18), Colors.transparent]),
          ),
          Column(
            children: [
              // progress
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            const Text(
                              'මෙම විධාන සංඥාවේ නිවැරදි සිංහල නම තෝරන්න:',
                              style: TextStyle(
                                  color: textPrimary,
                                  fontWeight: FontWeight.w900,
                                  fontSize: 16),
...
class _VAnswerTile extends StatelessWidget {
  final int index;
  final String text;
...
class _VQ {
  ...
  final int correct;
  _VQ({required this.item, required this.options, required this.correct});
}

// ----------------- Result Screen -----------------
class VidhanaQuizResult extends StatelessWidget {
  final int score;
  final int total;
  final List<ImageDetails> items;
  const VidhanaQuizResult(
      {super.key,
      required this.score,
      required this.total,
      required this.items});
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                          builder: (_) => VidhanaQuizScreen(items: items),
                        ),
                      );
                    },
