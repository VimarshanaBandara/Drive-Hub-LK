// traficlight_home.dart — Traffic Light Signals (රථ වාහන ආලෝක සංඥා)
// Light/White Theme + Favourite filter + Search + Quick filters + Smart Bottom Sheet
...
class ImageDetails {
  ...
  final String title; // Sinhala
  final String details; // English
  const ImageDetails({
...
// -------- Data (6 traffic light signals) --------
List<ImageDetails> _images = const [
  ImageDetails(imagePath: 'images/tr1.png', title: 'නතර කරනු', details: 'Stop'),
  ImageDetails(
  ...
  ImageDetails(imagePath: 'images/tr4.png', title: 'නතර කරනු', details: 'Stop'),
  ImageDetails(
...
class TraficLight_Home extends StatefulWidget {
  const TraficLight_Home({super.key});
  @override
  State<TraficLight_Home> createState() => _TraficLight_HomeState();
}

class _TraficLight_HomeState extends State<TraficLight_Home> {
  final _search = TextEditingController();
  // 0 All | 1 Stop | 2 Caution | 3 Proceed/Direction
  int _chip = 0;
  ...
  List<ImageDetails> get _filtered {
    ...
    switch (_chip) {
      case 1: // Stop (Red)
        src = src.where((e) =>
            e.title.contains('නතර') ||
            e.details.toLowerCase().contains('stop'));
        break;
      case 2: // Caution (Amber/Yellow)
        src = src.where((e) =>
            e.title.contains('අඩු') ||
            e.details.toLowerCase().contains('slow') ||
            e.details.toLowerCase().contains('carefully'));
        break;
      case 3: // Proceed / Direction (Green / Arrows)
        src = src.where((e) =>
            e.title.contains('ඉදිරියට') ||
            e.title.contains('දිශාව') ||
            e.details.toLowerCase().contains('go straight') ||
            e.details.toLowerCase().contains('drive safely') ||
  ...
  Widget build(BuildContext context) {
    ...
    const accent = Color(0xFF7C4DFF); // keep purple accent
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: const Text('රථ වාහන ආලෝක සංඥා',
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => TraficLightQuizScreen(items: _images),
            ),
          );
        },
      ...
      body: Column(
        children: [
          // Top summary card + Favourite toggle
          Padding(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(_onlyFavs ? 'Favourites' : 'Total Signals',
...
// ===================== CREATIVE QUIZ (TRAFFIC LIGHTS) =====================

class TraficLightQuizScreen extends StatefulWidget {
  final List<ImageDetails> items;
  const TraficLightQuizScreen({super.key, required this.items});

  @override
  State<TraficLightQuizScreen> createState() => _TraficLightQuizScreenState();
}

class _TraficLightQuizScreenState extends State<TraficLightQuizScreen> {
  // theme
  ...
  late final List<_TLQ> _questions;
  int _current = 0;
  int _score = 0;
  ...
  List<_TLQ> _buildQuestions(List<ImageDetails> items) {
    final rng = Random();

    // dedupe identical titles (Stop appears twice)
    final map = <String, ImageDetails>{};
    for (final it in items) {
      map[it.title] = it;
    }
    final base = map.values.toList()..shuffle(rng);

    final allTitles = map.keys.toList();
    final qs = <_TLQ>[];

    for (final correct in base) {
      final opts = <String>{correct.title};

      // add other real titles
      allTitles.shuffle(rng);
      for (final t in allTitles) {
        if (opts.length >= 4) break;
        if (t != correct.title) opts.add(t);
      }
      // top up with extras
      for (final d in _extraDistractors) {
        if (opts.length >= 4) break;
        opts.add(d);
      }

      final options = opts.toList()..shuffle(rng);
      final correctIdx = options.indexOf(correct.title);
      qs.add(_TLQ(item: correct, options: options, correct: correctIdx));
    }
    return qs;
  }
  ...
  void _onPick(int index) async {
    ...
    if (_current < _questions.length - 1) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (_) => TraficLightQuizResult(
            score: _score,
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            ...
            right: -40,
            child: _blob(220,
                [const Color(0xFFFFB703).withOpacity(.18), Colors.transparent]),
          ),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            const Text(
                              'මෙම ආලෝක සංඥාවේ නිවැරදි සිංහල නම තෝරන්න:',
                              style: TextStyle(
                                  color: textPrimary,
                                  fontWeight: FontWeight.w900,
                                  fontSize: 16),
...
class _TLAnswerTile extends StatelessWidget {
  final int index;
  final String text;
...
class _TLQ {
  ...
  final int correct;
  _TLQ({required this.item, required this.options, required this.correct});
}

// ----------------- Result Screen -----------------
class TraficLightQuizResult extends StatelessWidget {
  final int score;
  final int total;
  final List<ImageDetails> items;
  const TraficLightQuizResult(
      {super.key,
      required this.score,
      required this.total,
      required this.items});
        ...
        child: Column(
          children: [
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                          builder: (_) => TraficLightQuizScreen(items: items),
                        ),
                      );
                    },
            ...
            OutlinedButton.icon(
              ...
              label: const Text('Back to Signals'),
            ),
            const Spacer(),
            const Text(
