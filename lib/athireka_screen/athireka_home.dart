// athireka_home.dart — Light/White Theme + Favourite filter + Search + Quick filters
// + Creative Quiz (Hint, Skip, gradient header, small images)
import 'dart:math';
...
// -------- Data (6 supplementary/regulation plates) --------
...
class Athireka_Home extends StatefulWidget {
  const Athireka_Home({super.key});
  @override
  State<Athireka_Home> createState() => _Athireka_HomeState();
}

class _Athireka_HomeState extends State<Athireka_Home> {
  final _search = TextEditingController();
  // 0 All | 1 Distance/Length | 2 Parking Rules
  int _chip = 0;
  ...
  List<ImageDetails> get _filtered {
    ...
    switch (_chip) {
      case 1: // Distance/Length
        src = src.where((e) =>
            e.details.toLowerCase().contains('distance') ||
            e.details.toLowerCase().contains('length') ||
            e.title.contains('දුර') ||
            e.title.contains('දිග'));
        break;
      case 2: // Parking rules (start/direction/end/time)
        src = src.where((e) =>
            e.details.toLowerCase().contains('parking') ||
            e.details.toLowerCase().contains('standing') ||
            e.details.toLowerCase().contains('prohibition') ||
            e.title.contains('නවතා') ||
            e.title.contains('නැවැත්වීම') ||
            e.title.contains('තහනම්') ||
            e.title.contains('දිශාව') ||
            e.title.contains('අරම්භ') ||
            e.title.contains('අවසාන') ||
            e.title.contains('කාලසීමාව'));
        break;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: const Text('නියාමන/අතිරේක පුවරු',
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => AthirekaQuizScreen(items: _images),
            ),
          );
        },
...
// ===================== CREATIVE QUIZ (ATHIREKA) =====================

class AthirekaQuizScreen extends StatefulWidget {
  final List<ImageDetails> items;
  const AthirekaQuizScreen({super.key, required this.items});

  @override
  State<AthirekaQuizScreen> createState() => _AthirekaQuizScreenState();
}

class _AthirekaQuizScreenState extends State<AthirekaQuizScreen> {
  // theme
  ...
  List<_Q> _buildQuestions(List<ImageDetails> items) {
    ...
    final total = min(6, pool.length); // small set -> 6 Qs
    final qs = <_Q>[];
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            const Text(
                              'මෙම අතිරේක පුවරුවේ සිංහල නම තෝරන්න:',
                              style: TextStyle(
                                  color: textPrimary,
                                  fontWeight: FontWeight.w900,
                                  fontSize: 16),
...
// ----------------- Result Screen -----------------
class AthirekaQuizResult extends StatelessWidget {
  final int score;
  final int total;
  final List<ImageDetails> items;
  const AthirekaQuizResult(
      {super.key,
      required this.score,
      required this.total,
      required this.items});
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                          builder: (_) => AthirekaQuizScreen(items: items),
                        ),
                      );
                    },
