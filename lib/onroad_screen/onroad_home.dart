// roadmark_home.dart — Light/White Theme + Favourite filter + Search + Quick filters + Smart Bottom Sheet
// + Creative Quiz (hint toggle, skip, gradient header, letter badges, compact image)
...
List<ImageDetails> _images = const [
  ...
  ImageDetails(
      ...
      title:
          'සීග්‍රගාමී හා අධිවේගී මාර්ග සඳහා පරාවර්තනය වන බුබුළු යොදා කඩ ඉරි මධ්‍ය රේඛාව සලකුණු කිරීම  ',
      details:
  ...
  ImageDetails(
      ...
      title:
          'කඩ ඉරි මධ්‍ය රේඛාව නිරන්තරව අඛණ්ඩ රේඛාව සහ සමාන්තරවීම සමග සංයෝජනය වීම.',
  ...
  ImageDetails(
      ...
      title:
          'රථ වාහන ගාල් කිරීම තහනම් කර ඇති බව හෝ සීමා කර ඇති බව දැක්වීමට ධාවන පථයේ අයින දිගේ පිහිටුවන රේඛාව.',
  ...
  ImageDetails(
      ...
      details: 'SLOW'),
  ImageDetails(imagePath: 'images/r25.png', title: 'නවතිනු', details: 'STOP'),
  ImageDetails(
...
class Roadmark_home extends StatefulWidget {
  const Roadmark_home({super.key});
  @override
  State<Roadmark_home> createState() => _Roadmark_homeState();
}

class _Roadmark_homeState extends State<Roadmark_home> {
  final _search = TextEditingController();
  // 0 All | 1 Lines | 2 Lanes/Boxes | 3 Words/Arrows | 4 Priority/Crossings
  int _chip = 0;
  ...
  List<ImageDetails> get _filtered {
    ...
    switch (_chip) {
      case 1: // Lines (centre/edge/double/reflective/hatch/no parking lines)
        src = src.where((e) {
          final d = e.details.toLowerCase();
          return d.contains('line') ||
              d.contains('centre') ||
              d.contains('center') ||
              d.contains('edge') ||
              d.contains('stud') ||
              d.contains('hatch') ||
              e.title.contains('රේඛාව') ||
              e.title.contains('රේඛා');
        });
        break;
      case 2: // Lanes / Boxes
        src = src.where((e) {
          final d = e.details.toLowerCase();
          return d.contains('lane') ||
              d.contains('lanes') ||
              d.contains('box') ||
              e.title.contains('තීරු') ||
              e.title.contains('බස්රථ') ||
              e.title.contains('කොටුව');
        });
        break;
      case 3: // Words / Arrows (STOP, SLOW, Give Way, Compulsory turns)
        src = src.where((e) {
          final d = e.details.toLowerCase();
          return d.contains('stop') ||
              d.contains('slow') ||
              d.contains('give way') ||
              d.contains('compulsory') ||
              e.title.contains('නවතිනු') ||
              e.title.contains('සෙමෙන්') ||
              e.title.contains('මාර්ගය ඉඩ') ||
              e.title.contains('අනිවාර්යෙන්');
        });
        break;
      case 4: // Priority / Crossings (stop/give way lines, pedestrian)
        src = src.where((e) {
          final d = e.details.toLowerCase();
          return d.contains('stop line') ||
              d.contains('give way') ||
              d.contains('pedestrian') ||
              e.title.contains('පදික');
        });
        break;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: const Text('මාර්ගය මත පිහිටුවන සලකුණු',
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => RoadmarkQuizScreen(items: _images),
            ),
          );
        },
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(_onlyFavs ? 'Favourites' : 'Total Markings',
...
// ===================== CREATIVE QUIZ (ROADMARKS) =====================

class RoadmarkQuizScreen extends StatefulWidget {
  final List<ImageDetails> items;
  const RoadmarkQuizScreen({super.key, required this.items});

  @override
  State<RoadmarkQuizScreen> createState() => _RoadmarkQuizScreenState();
}

class _RoadmarkQuizScreenState extends State<RoadmarkQuizScreen> {
  // theme
  ...
  List<_Q> _buildQuestions(List<ImageDetails> items) {
    ...
    final total = min(12, pool.length); // 12 Qs for this set
    final qs = <_Q>[];
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            const Text(
                              'මෙම මාර්ග සළකුණේ (On-road marking) සිංහල නම තෝරන්න:',
                              style: TextStyle(
                                  color: textPrimary,
                                  fontWeight: FontWeight.w900,
                                  fontSize: 16),
...
// ----------------- Result Screen -----------------
class RoadmarkQuizResult extends StatelessWidget {
  final int score;
  final int total;
  final List<ImageDetails> items;
  const RoadmarkQuizResult(
      {super.key,
      required this.score,
      required this.total,
      required this.items});
        ...
        child: Column(
          children: [
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                          builder: (_) => RoadmarkQuizScreen(items: items),
                        ),
                      );
                    },
            ...
            OutlinedButton.icon(
              ...
              label: const Text('Back to Markings'),
            ),
            const Spacer(),
            const Text(
