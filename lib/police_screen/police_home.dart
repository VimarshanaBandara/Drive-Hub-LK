// police_home.dart — Light/White Theme + Favourite filter + Search + Quick filters + Smart Bottom Sheet
...
class _Police_homeState extends State<Police_home> {
  ...
  int _chip = 0; // 0 All | 1 Stop | 2 Forward | 3 Behind/Both
  ...
  List<ImageDetails> get _filtered {
    ...
    switch (_chip) {
      case 1: // Stop
        src = src.where((e) =>
            e.title.contains('නවති') ||
            e.title.contains('නතර') ||
            e.details.toLowerCase().contains('stop'));
        break;
      case 2: // Forward
        src = src.where((e) =>
            e.title.contains('එන්න') ||
            e.details.toLowerCase().contains('forward'));
        break;
      case 3: // Behind / Both
        src = src.where((e) =>
            e.title.contains('පිටුපසින්') ||
            e.title.contains('ඉදිරියෙන් හා පිටුපසින්') ||
            e.details.toLowerCase().contains('behind') ||
            e.details.toLowerCase().contains('both'));
        break;
    }
  ...
  Widget build(BuildContext context) {
    ...
    const accent = Color(0xFF7C4DFF); // keep same accent (purple)
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: const Text('පොලිස් හස්ත සංඥා',
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => PoliceQuizScreen(items: _images),
            ),
          );
        },
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                _quickAction(
                  icon: Icons.no_stroller_rounded, // playful stop-like
                  label: 'Stop',
                  selected: _chip == 1,
...
// ===================== CREATIVE QUIZ (HAND SIGNALS) =====================

class PoliceQuizScreen extends StatefulWidget {
  final List<ImageDetails> items;
  const PoliceQuizScreen({super.key, required this.items});

  @override
  State<PoliceQuizScreen> createState() => _PoliceQuizScreenState();
}

class _PoliceQuizScreenState extends State<PoliceQuizScreen> {
  // theme constants
  ...
  // extra Sinhala distractors to diversify options
  static const List<String> _extraDistractors = [
    'වේගය අඩු කරන්න',
    'වමට හැරෙන්න',
    'දකුණට හැරෙන්න',
    'පදිකයින් නවතන්න',
    'රථ වාහන ඉදිරියට යන්න'
  ];
  ...
  List<_Q> _buildQuestions(List<ImageDetails> items) {
    ...
    // unique by title (handles the duplicate 'ඉදිරියට එන්න')
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            const Text(
                              'මෙම පොලිස් හස්ත සංඥාවේ නිවැරදි සිංහල නම තෝරන්න:',
                              style: TextStyle(
                                  color: textPrimary,
                                  fontWeight: FontWeight.w900,
                                  fontSize: 16),
...
// ----------------- Result Screen -----------------
class PoliceQuizResult extends StatelessWidget {
  final int score;
  final int total;
  final List<ImageDetails> items;
  const PoliceQuizResult(
      {super.key,
      required this.score,
      required this.total,
      required this.items});
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                          builder: (_) => PoliceQuizScreen(items: items),
                        ),
                      );
                    },
